...
public class AuthService {
	...
	public void createMember(Member member) {
		/*
		 * 파라미터의 member 객체에는 아이디, 비밀번호, 이름, 휴대폰 번호, 이메일 등의 정보가 담겨있다. 따라서 mrole, menable의
		 * 값을 셋팅해주면 된다. '회원가입' 부분에 있어서 '운영진' 로그인 페이지와 '교육생' 로그인 페이지를 분리시킬지 고려해야 함. -->
		...
		/*
		 * 가입하는 회원에 대한 권한 부여 회원가입은 '운영진'만 가능하도록 설계함. 따라서 '운영진'의 권한을 얻으려면 앞 글자는 'kosa'를
		 * 붙여야 함. Ex) kosa001
		 */
	...
	public Boolean idCheck(String mid) {
		...
		String id = memberDao.checkMid(mid);

		// 중복 아이디 검사
		if (mid.equals(id)) {
			log.info("중복된 아이디가 있음");
			idCheck = true;
		} else {
			log.info("중복된 아이디가 없음!");
			idCheck = false;
		}
		return idCheck;
	}
	...
	public Boolean emailCheck(String memail) {
		...
		String email = memberDao.checkMemail(memail);
	...
	public Boolean phoneCheck(String mphone) {
		...
		String phone = memberDao.checkMphone(mphone);
	...
	public Map<String, String> memberCheck(String mid, String mpassword) {
		...
		// 1. loadUserByUsername(mid)는 DB로부터 사용자가 입력한 mid에 해당하는 Member 객체를 가져온다. (아이디와
		// 권한의 정보가 담겨있음)
